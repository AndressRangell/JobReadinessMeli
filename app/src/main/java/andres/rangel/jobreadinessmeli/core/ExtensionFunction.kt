...
fun String.changeStateFavorite(binding: ActivityProductDetailsBinding, context: Context) {
    val preferences: SharedPreferences =
        context.getSharedPreferences("IDS", Context.MODE_PRIVATE) ?: return
...
fun Context.getFavorites(): String {
    val preferences: SharedPreferences =
        this.getSharedPreferences("IDS", Context.MODE_PRIVATE) ?: return ""
    var favoritesId = ""
    preferences.all.values.forEach { favoritesId += "$it," }
    return favoritesId
}